def execution(at,bt,n,it,quant):
    ...
    for i in range(len(at)):
        preAt.append(at[0])
    while not(len(at)==0) or not(len(waitQ)==0)or not(len(aux)==0):
        if not(len(aux)==0):
                 if timer>=aux[1] and aux[5]>=aux[2]:
                     timer=timer+aux[2]
                     print('process ',aux[0], 'completed in auxilary queue at ',timer)
                     for j in range(6):
                        del aux[0]
                    ...
                    preAt.append(aux[4])
                    qtime.append(aux[5])
                    print('process ',aux[0], 'executing in auxilary queue in ',timer)
                    for j in range(6):
                        del aux[0]

        if not(len(waitQ)==0):

                if timer>=waitQ[1] and waitQ[5]<quant:
                    aux.append(waitQ[0])
                    aux.append(waitQ[1])
                    aux.append(waitQ[1+1])
                    aux.append(waitQ[3])
                    aux.append(waitQ[4])
                    aux.append(waitQ[5])
                    for j in range(6):
